...
| **Director** (orquestador) | `anthropic/claude-opus-5.5` | Primero mira todo el material (cuadros de cada video con su segundo, fotos), infiere la intención, propone la historia, cataloga las tomas y le da al Oído un **encargo** (qué escuchar para que esa historia funcione). Después monta: es el único "escritor" del montaje (principio *single writer* de la investigación 02). Monta **en la misma conversación** en la que planificó, así conserva lo que vio y decidió. |
| **Oído** | `google/gemini-3.8-flash` | Acepta audio como entrada (también video, imágenes y texto). Escucha el tema entero con el encargo del Director y produce el mapa musical: secciones, energía, hit points, ritmo de corte, cómo fluye el tema y respuestas a las preguntas del Director. Recibe la grilla automática para ser preciso, pero puede corregirla (tempo doble/mitad, pulso libre). |
...
- **Los videos nunca viajan al servidor**: el navegador extrae hasta 4 cuadros por video (con su segundo exacto) y reduce las fotos. El Director elige el segundo de inicio de cada toma mirando esos cuadros.
...
1. **Crítico que mira el resultado**: renderizar el video y pasárselo a Gemini (acepta video con audio) para que evalúe si los cortes "se sienten" en el golpe, y devolverle notas al Director (patrón evaluador–optimizador).
2. **Detección de movimiento en las tomas**: medir en el navegador dónde hay más acción en cada video para ofrecerle al Director fragmentos candidatos, no solo 4 cuadros.
